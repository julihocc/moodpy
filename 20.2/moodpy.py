...
def format_num(n, places=3):
    ...
    l = len(n)
    s = ""
    for _ in range(places-l):
        s+="0"
    return s+n

def ExeToMoodle(foldername, mihtml, mifb=[], penal=0.5):
...
def pretty(exercise, retroalimentacion=None):
...
def master(generador, ubicacion, cuantos, cabecera):
    assert cuantos > 0
    mihtml = []
    for k in range(cuantos):
        ejercicio = generador(cabecera)
        mihtml.append(ejercicio)
    ExeToMoodle(ubicacion, mihtml)

def quick(generador, ubicacion="./", cuantos=0, xml=None, cabecera="Empty header"):
    if xml==True:
        master(generador, ubicacion, cuantos, cabecera)
    elif xml==False:
